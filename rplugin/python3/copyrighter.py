...
class User:
    """
    User contains information about the file owner
    """
...
class CopyrighterPlugin:
    def __init__(self, nvim):
        ...
        for section in cnfg.sections():
            for extension in cnfg[section]["Extension"].split(" "):
                header = open(
                    os.path.join(cnfg_path, cnfg[section]["File"]), "r"
                )
    ...
    def _context(self, srcfile: str):
        ...
        context["user"] = User(
            name=self.nvim.eval("g:header_name"),
    ...
    def header_command(self, srcfile: str):
        """
    ...
    @neovim.autocmd(
        "BufNewFile", pattern=r"*", eval='expand("<afile>")', sync=True
    )
